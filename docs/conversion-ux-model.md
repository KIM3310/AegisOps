# Conversion UX Model - AegisOps -- Multimodal Incident Review System
...
| Primary buyer or user | SOC leads, IT operations managers, MSP incident teams, and infrastructure owners with noisy handoffs. |
| Value wedge | A replayable incident review cockpit that turns logs and screenshots into structured handoff evidence. |
...
| First action | Open the review guide, run or inspect Replay eval suite passes, and map one buyer workflow to the pilot checklist. |
...
- Put Time-to-report reduction on the pilot scorecard.
